...
{ pkgs ? import <nixpkgs> { config.allowUnfree = true; } }:  # unfree: ngrok

pkgs.mkShell {
  packages = with pkgs; [ python3 ffmpeg libopus ngrok ];
  ...
  shellHook = ''
    ...
    if [ ! -f "$dir/.env" ]; then
      ...
      echo "created voice-agent/.env from .env.example"
    fi
    echo "voice-agent shell ready (python from .venv, ffmpeg + opus + ngrok from nix)"
    (cd "$dir" && python doctor.py) || true   # go-live checklist against .env
  '';
}
